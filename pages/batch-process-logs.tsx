...
import { addDeviceGroup, deleteDeviceGroup, getDeviceGroups, getBatchLogs, pingPi, getRelatedLogs } from '@app/api/log';
...
class Homepage extends React.Component<IBasicPageProps, IDashboardPageState> {
  static getInitialProps = async (args) => {
    const { query: { q, page, search, groupTaskId }, asPath, req, initialState: {} } = args;
  ...
  search = async () => {
      const params = {
          ...
          groupTaskId: this.state.groupTaskId,
      };
      const rp = await getBatchLogs(params);
      this.setState({
          ...
          pageIndexRelatedMeterPi: Math.floor((rp?.offset + 19) / 20) + 1,
  ...
  render() {
    const { t } = this.props;
    return (
      <TotPage title={'Batch process logs'}>
              ...
              {!this.state.piEdit ?
                <>
                  <div style={{ padding: '15px', paddingTop: '20px', paddingBottom: '0px', fontWeight: 400, fontSize: '18px', color: '#000000' }}>
                    <h5 className="site-title">{'Batch process logs'}</h5>
                  </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', overflowX: 'auto', width: '100%' }}>
                      <table className="table">
                          <thead className="thead-dark">
                              <tr>
                                  <th className="table-th" scope="col">Batch ID</th>
