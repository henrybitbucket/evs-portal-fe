...
import { deleteProjectTag, getProjectTags, saveProjectTag } from '@app/api/log';
...
class Homepage extends React.Component<IBasicPageProps, IDashboardPageState> {
  ...
  static getInitialProps = async (args) => {
    ...
    let rp = await getProjectTags(params, req);
          ...
          <PageContent>
            ...
            >
              <div style={{
                ...
                <h5 className="site-title">{'Project Tag'}</h5>
              </div>
              <div style={{
                display: 'flex',
                padding: '30px',
                ...
                <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', width: '100%' }}>
                  <table className="table">
                    ...
                    <tr>
                      ...
                      <th className="table-th" scope="col">Tag Name</th>
                      <th className="table-th" scope="col">Tag Description</th>
                      <th className="table-th" scope="col">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                    {(this.state.tags || []).map((it, index) => (
                      <tr
                        key={index}
                        style={{
                          cursor: 'pointer',
                              ...
                              <div
                                ...
                                onClick={async () => {
                                  ...
                                  if (result && result.isConfirmed) {
                                    ...
                                    let rp = await deleteProjectTag(params);
                                    if (rp.success) {
                                      Swal.fire({
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <span style={{
                        ...
                        display: 'flex',
                        textAlign: 'center'
                      }}>{!this.state.tagId ? 'ADD NEW TAG' : 'EDIT TAG'}</span>
                    </div>
                    <div style={{
                      overflow: 'auto',
                      width: '100%',
                      ...
                      >
                        ...
                        />
                        {this.state.errorTagDescription && (
                          <small
                            className="help-block"
                            style={{
                              color: '#eb0000',
                    ...
                    <div style={{ marginTop: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <div
                        ...
                        onClick={async () => {
                          if (!!this.state.tagName && this.state.tagDescription) {
                            this.setState({
                            ...
                            let rp = await saveProjectTag(params);
                            if (rp.success) {
                              Swal.fire({
                            ...
                            if (!this.state.tagName) {
                              this.setState({ errorTagName: "Tag name can not be empty" });
                            }
                            if (!this.state.tagDescription) {
                              this.setState({ errorTagDescription: "Tag description can not be empty" });
                            }
                          }
                        }}
                      >
