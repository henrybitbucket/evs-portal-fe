...
import { createRoles, deleteRole, getPermission, getRoles, updateRoles, } from '@app/api/log';
import ReactTooltip from "react-tooltip";
import { Input } from 'reactstrap';
import { getUserRoleLogging } from "@app/api/user";
...
class Homepage extends React.Component<IBasicPageProps, IDashboardPageState> {
  ...
  searchGroup = async () => {
    const params = {};
    const rp = await getUserRoleLogging(params);
    this.setState({
      role: rp?.response.results || [],
    });
    console.log("roles aaa", this.state.role);
    if (this.state.role != null) {
      this.state.role.map((p, id) => {
        if (p.name === 'SUPER_ADMIN' || p.name === (getCookie('app_code') + '_SUPER_ADMIN')) {
              ...
              {!this.state.piAdd && !this.state.param && !this.state.task ?
                <>
                  <div style={{
                    ...
                    <h5 className="site-title">{'Role Management'}</h5>
                  </div>
                  <div style={{
                    display: 'flex',
                    padding: '30px',
                                    ...
                                    <>
                                      ...
                                      <div
                                        ...
                                        onClick={() => {
                                          ...
                                          this.setState({
                                            permissionAdded: [].concat(it.permissions || []),
                                            selectedRows: [].concat(it.permissions || []),
                                            roleId: it.id,
                                            roleName: it.name,
                                            desc: it.desc,
                                            showEditRole: true
                                      ...
                                      <div
                                        ...
                                        onClick={async () => {
                                          ...
                                          if (result && result.isConfirmed) {
                                            ...
                                            let rp = await deleteRole(params);
                                            console.log(rp);
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      >
                        ...
                        <span style={{
                          ...
                          fontSize: '12px',
                          marginTop: '5px'
                        }}>{!this.state.roleName ? this.state.errorRoleName : ''}</span>
                      </div>
                      ...
                      >
                        ...
                        />
                        <span style={{
                          display: this.state.errorDescription != '' ? 'flex' : 'none',
                          color: 'red',
                          fontSize: '12px',
                          marginTop: '5px'
                        }}>{!this.state.desc ? this.state.errorDescription : ''}</span>
                    ...
                    <div style={{ marginTop: '20px', marginLeft: '40px' }}>
                      <div
                        ...
                        onClick={async () => {
                          if (this.checkInput()) {
                            this.setState({ errorRoleName: "", loading: true });
                            this.setState({ errorDescription: "" });
                            ...
                            let rp = await createRoles(params);
                            ...
                            if (this.state.roleName == null || this.state.roleName == "") {
                              this.setState({ errorRoleName: "Role name can not be empty" });
                            }
                            if (this.state.desc == null || this.state.desc == "") {
                              this.setState({ errorDescription: "Description can not be empty" });
                            }
                          }
                        }}
                      >
              ...
              {!!this.state.showEditRole && (
                ...
                >
                  <div style={{
                    ...
                    <div style={{ width: '100%', marginTop: '20px' }}>
                      ...
                      <div style={{
                        ...
                        <table className="table">
                          <thead className="thead-dark">
                          <tr>
                            <th className="table-th table-th-50" scope="col"/>
                            <th className="table-th table-th-50" scope="col" style={{ textAlign: 'center' }}>
                              ID
                            </th>
                            <th className="table-th" scope="col" style={{ textAlign: 'center' }}>
                              NAME
                          ...
                          <tbody>
                          {((this.state.permissionAdded || []).concat((this.state.addPermission || []).filter(item => !(this.state.permissionAdded.map(it => it.id) || []).includes(item.id))) || [])
                            .map((it, index) => {
                              return (
                                <tr
                                  key={index}
                    ...
                    <div style={{ display: 'flex', marginTop: '25px', alignItems: 'center', justifyContent: 'center' }}>
                      <div
                        style={{ marginLeft: '15px' }}
                        className="button-btn w-80"
                        onClick={async () => {
                          if (this.checkInput()) {
                            ...
                            let rp = await updateRoles(params);
                            console.log(rp);
