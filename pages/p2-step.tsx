...
class Homepage extends React.Component<IBasicPageProps, IDashboardPageState> {
  ...
  addRemoveJOb = async (index) => {
    ...
    console.info('addRemoveJOb', this.state.job.items);
  ...
  reloadJob = async () => {
    ...
    await this.setState({filterList: {}, searchTBMSN: ''})
    this.state.job.items = [];
    await this.setState({action: "RELOAD"})
    this.setState({isLoading: true});
    let rp = await getP2Jobs({
      jobNo: this.state.jobName,
      p2Worker: this.state.jobBy || this.state.selected?.worker?.email,
    });
    ...
    this.state.contractOrder = this.state.job.contractOrder;
    this.state.jobBy = this.state.job.jobBy || this.props.userInfo.email;
    this.setState({isLoading: false});
    ...
    for (let i = 0; i < this.state.job.items?.length; i++) {
      this.state.job.items[i]._ref && this.state.job.items[i]._ref.setCheckCoupleTimeout();
  ...
  async componentDidMount() {
    ...
    const rpPermissions = await getUserPermissions(params);
    let permissions = rpPermissions?.response.results[0].permissions || [];
    ...
    isWorker = isWorker || !!permissions.filter(p => p.name === 'P_P2_WORKER')[0];
    ...
    // if isWorker -> isManager set = false;
    // https://powerautomationsg.atlassian.net/browse/MMS-207
    if (!!isWorker) {
      isManager = false;
      workers = [{...this.state.userInfo}]
    }
    ...
    if (this.state.selected?.worker?.email) {
      let allP2Jobs = await getP2Jobs({jobNo: "all", msn: this.state.searchMSNALL, p2Worker: this.state.selected?.worker.email, contractOrder: this.state.searchContractOrder});
      this.setState({ allP2Jobs: allP2Jobs?.response || [] });
    }
  }

  searchTBMSN = async () => {
    let filterList = (this.state.job?.items || []).filter(it => {
      if (!this.state.searchTBMSN || !this.state.searchTBMSN.trim()) {
        return true;
      }
      return (it.msn || '').trim().toLowerCase().indexOf(this.state.searchTBMSN.trim().toLowerCase()) > -1
    })

    let ids = {}
    filterList.map(it => ids[it._tmpIdx] = ('---> ' + it.msn));
    this.setState({keyRefresh: Date.now(), filterList: {items: filterList, ids}}, () => {
      let filterTotalPages = Math.floor(((this.state.filterList?.items?.length || 0) + (this.state.pageSize - 1)) / this.state.pageSize);
      let filterTotalRecords = this.state.filterList?.items?.length || 0;
      this.setState({
        filterTotalPages,
        filterTotalRecords,
        pageIndex: 1,
  ...
  exportCSV = async (rows) => {
    ...
    link.setAttribute("download", this.state.jobName + '.' + Date.now() + ".csv");
  ...
  render() {
    ...
    console.info('this.state.selected?.isManager', this.state.selected?.isManager)

    return (
      <TotPage title={'P2 Provisioning Job Sheet'}>
            ...
            >
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{display: 'flex', flexDirection: 'column', flex: 1, padding: '10px 15px'}}>
                    <span style={{fontWeight: 400, fontSize: '18px', color: '#000000', margin: 'unset'}} className="site-title">{'P2 Provision'} <i className="fa fa-caret-right ml-5 mr-5"/> {'P2 Provisioning Job Sheet'}</span>
                    ...
                    <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: '15px'}}>
                      ...
                      <>
                        <span style={{minWidth: '130px', maxWidth: '130px',}}>P2 Group: </span>
                        <SelectCustom
                          field={{
                              name: "job-select"
                          }}
                          ...
                          groupstyle={{ marginLeft: '15px', marginBottom: '0px', width: '250px'}}
                        />
                      </>
                      )}
                    </div>
                    ...
                    <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: '15px'}}>
                      {(this.state.selected?.isManager || this.state.selected?.isPAStaff) && (
                      <>
                        <span style={{minWidth: '130px', maxWidth: '130px',}}>P2 worker: </span>
                        <SelectCustom
                          field={{
                              name: "job-select"
                          }}
                          ...
                          onChange={async (field, value) => {
                            ...
                            this.setState({
                              ...
                              await this.setState({isLoading: true, selectJobName: null, jobBy: this.state.selected?.worker?.email,})
                              let allP2Jobs = await getP2Jobs({
                                jobNo: "all",
                                msn: this.state.searchMSNALL,
                                searchJobKey: Date.now(),
                                p2Worker: this.state.selected?.worker?.email,
                                contractOrder: this.state.searchContractOrder,
                              });
                              this.setState({ allP2Jobs: allP2Jobs?.response || [], isLoading: false });
                            })
                  ...
                  <div style={{flex: 1, pointerEvents: !this.state.selected?.worker?.email ? 'none' : undefined}}>
                    <table>
                      <tr>
                        <td style={{paddingTop: '10px'}}>
                          <input
                            ...
                            value={this.state.searchMSNALL}
                            onChange={(event) => {
                              this.setState({ searchMSNALL: event.target.value });
                            }}
                            placeholder={'Search Meter SN all'}
                          />
                        </td>
                      </tr>
                      <tr>
                        <td style={{paddingTop: '10px'}}>
                          <input
                            ...
                            value={this.state.searchContractOrder}
                            onChange={(event) => {
                              this.setState({ searchContractOrder: event.target.value });
                            }}
                            placeholder={'Search Contract Order'}
                          />
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <div
                            ...
                            style={{width: 'fit-content', padding: '15px', float: 'right', marginBottom: '5px', marginTop: '5px'}}
                          >
                            <span>{'SEARCH'}</span>
                          </div>
                        </td>
                      ...
                      <tr>
                        <td>
                            <SelectCustom
                              ...
                              name="job-select"
                              key={this.state.searchJobKey || Date.now()}
                              options={[
                                ...(
                              ...
                              value={this.state.selectJobName}
                              onChange={async (field, value) => {
                                  this.setState({ selectJobName: value.value, jobName: value.value, jobBy: this.state.selected?.worker?.email }, async () => {
                  ...
                  <div style={{pointerEvents: !this.state.selected?.worker?.email ? 'none' : undefined}}>
                    <table>
                      <tr>
                        <td>
                      ...
                      <tr>
                        ...
                        <td>
                          <input
                            ...
                            value={this.state.contractOrder || ""}
                            onChange={(event) => {
                              this.setState({ contractOrder: event.target.value });
                            }}
                            placeholder={'Contract Order'}
                          />
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td style={{paddingTop: '10px'}}>
                          <input
                            ...
                            // value={this.state.job?.createDate ? moment(this.state.job.createDate).format('YYYY-MM-DD HH:mm:ss') : ""}
                            disabled={true}
                          />
                        </td>
                      </tr>
                ...
                <div className="table-wrap">
                  <table className="table" style={{marginTop: '15px', pointerEvents: !!this.state.openSubmitReport ? 'none' : null,}}>
                    <thead className="thead-dark">
                      <tr>
                        ...
                        <th className="table-th">
                          <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center'}}>
                            ...
                            <input
                              ...
                              value={this.state.searchTBMSN}
                              onChange={(event) => {
                                this.setState({ searchTBMSN: event.target.value }, async () => {
                                  this.searchTBMSN();
                                });
                              }}
                              placeholder={'Filter...'}
                        ...
                        <th className="table-th">DECOUPLE</th>
                        <th className="table-th">ON BOARDING CHECK</th>
                    ...
                    <tbody>
                      {(this.state.filterList?.items || [])
                        .concat((this.state.job.items || []).filter(it => {
                          return !(this.state.filterList?.ids || {})[it._tmpIdx]
                        }))
                        ...
                        .map((it, idx) => {
                          ...
                          // let idxAll = idx + ((this.state.pageIndex || 0) - 1) * (this.state.pageSize || 10);
                          let idxAll = idx;

                          // filter
                          let filterMSN = !this.state.searchTBMSN;
                          if (!!this.state.searchTBMSN) {
                            filterMSN = (it.msn || '').trim().toLowerCase().indexOf(this.state.searchTBMSN.trim().toLowerCase()) > -1
                          }
                          return <ItemCheck
                            {...this.props}
                            ...
                            hide={!(idx >= activeIndexFrom && idx <= activeIndexTo) || !filterMSN}
                            seq={it._tmpIdx}
                            ref={ref => it._ref = ref}
                            data={it}
                            size={this.state.job.items.length}
                            tKey={(this.state.keyRefresh || '') + '_' + it._tmpIdx}
                            key={(this.state.keyRefresh || '') + '_' + it._tmpIdx}
                            ...
                            removeItem={async () => {
                              await this.addRemoveJOb(it._tmpIdx - 1);
                              this.searchTBMSN();
                            }}
                          />
                      })}
                  ...
                  <div className="col-sm-12 col-md-7 pagination" style={{ justifyContent: 'flex-end', marginBottom: '25px' }}>
                    {(!(this.state.searchTBMSN || '').trim().length && this.state.totalRecords > 1 && this.state.totalPages >= 2) &&
                      <Pagination
                    ...
                    {(!!(this.state.searchTBMSN || '').trim().length && this.state.filterTotalRecords > 1 && this.state.filterTotalPages >= 2) &&
                      <Pagination
                        href={this.props.asPath.replace(/\?.*/g, '')}
                        totalRecords={this.state.filterTotalRecords}
                        totalPages={this.state.filterTotalPages}
                        onPageChanged={(query) => {
                          this.setState({pageIndex: query.pageIndex});
                        }}
                        pageIndex={this.state.pageIndex}
              ...
              <div
                style={{
                  ...
                  pointerEvents: (!!this.state.openSubmitReport || !!this.state.batchCoupleDevicesError?.show) ? 'none' : null
                }}
              >
                {(!!this.state.selected?.worker?.email && this.state.selected?.worker?.email === this.state.userInfo?.email) && (
                <>
                  <ReactTooltip globalEventOff="click" place="bottom" type="info" effect="solid"/>
                  <div
                    data-tip="Click to create new job"
                    className="button-btn w-160 ml-10"
                    onClick={async () => {
                      this.state.job.items = [];
                      await this.setState({action: "new", filterMSN: {}, searchTBMSN: ''})
                      let rp = await getP2Jobs({p2Worker: this.state.selected?.worker?.email});
                      this.state.job = rp?.response || {};
                      this.state.job.items = [];
                      this.state.jobName = this.state.job.name;
                  ...
                  <div
                    ...
                    onClick={async () => {
                      ...
                      this.setState({isLoading: true});
                      let params = {};
                      params.name = this.state.job.name;
                      params.contractOrder = this.state.contractOrder;
                      params.title = this.state.job.title;
                      params.jobBy = this.state.jobBy || this.state.selected?.worker?.email;
                      params.items = [];
                      (this.state.job.items || [])
                        .map((it, idx) => {
                      ...
                      let rp = await saveP2Job(params);
                      this.setState({isLoading: false});
                      if (rp.success) {
                        Swal.fire(
                          'Information!',
                          rp.success == true ? 'Save Successfully!' : (rp.message || rp.errorDescription),
                          'success',
                        );
                      } else {
                        Swal.fire(
                ...
                <div
                  ...
                  onClick={async () => {
                    ...
                    let head = ['Seq', 'Meter SN', 'MCU SN', 'Couple State', 'Result'];
                    let rows = [head];
                    (this.state.job.items || [])
                      .map((it, idx) => {
                        let obj = it._ref.collectDataSubmit();
                        rows.push([it._tmpIdx,
                        it.msn,
                        it.sn,
                        obj.isCoupled ? 'COUPLED' : 'NOT-COUPLE',
                        obj?.isPassed === true ? 'PASS' : obj?.isPassed === false ? 'FAIL' : 'None',
                        ])
                      })
                    this.exportCSV(rows);
                  }}
                ...
                {(!!this.state.selected?.isWorker && !this.state.selected?.isManager && !this.state.selected?.isPAStaff) && (
                <>
                  <ReactTooltip globalEventOff="click" place="bottom" type="info" effect="solid"/>
                  <div
                    data-tip="Click to batch couple the un-couple meter and MCU pairs in this job"
                    className="button-btn ml-10"
                    onClick={async () => {
                      ...
                      await saveP2Job(params);

                      const list = [];
                      for (let i = 0; i < this.state.job.items?.length; i++) {
                        let it = this.state.job.items[i];
                        if (it.tempDataChecks?.isCoupled === false) {
                          const map = {
                            sn: it.sn,
                            msn: it.msn
                          }
                      ...
                      if (list.length > 0) {
                        const response = await batchCoupleDevices(list);
                        if (response?.success) {
                        ...
                        await this.setState({isLoading: false});
                        await Swal.fire({
                          html: `<p style='text-align: center; font-size: 14px;'>${'There is no UN-COUPLE devices'}</p>`,
                          icon: 'success',
                          confirmButtonText: 'OK',
                        });
                      }
                  ...
                  >
                    <span>{'BATCH COUPLE'}</span>
                  </div>
                  <ReactTooltip globalEventOff="click" place="bottom" type="info" effect="solid"/>
                  <div
                    data-tip="Click to onboarding check"
                    className="button-btn ml-10"
                    onClick={async () => {

                      await this.setState({action: "batchOnboardingCheck", pageIndex: 1})
                      for (let i = 0; i < this.state.job.items?.length; i++) {
                        let it = this.state.job.items[i];
                        it._ref.state.action = 'onboardingCheck';
                        await it._ref.onboardingCheck();
                        await it._ref.setState({isLoading: false});
                      }
                    ...
                    style={{width: 'fit-content', padding: '15px'}}
                  >
                    <span>{'BATCH ONBOARDING CHECK'}</span>
                  </div>
                </>
                )}
                ...
                <ReactTooltip globalEventOff="click" place="bottom" type="info" effect="solid"/>
                <div
                  data-tip="Click to get review report"
                  className="button-btn w-160 ml-10"
                  onClick={() => {
                    if (!this.state.jobName) {
                      Swal.fire(
                    ...
                    require('@app/utils/next-routes').Router.pushRoute(`/p2-provisioning-report?jobNo=${this.state.jobName}`)
                  }}
                >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        for (let i = 0; i < this.state.job.items?.length; i++) {
                          ...
                          obj.managerSubmit = this.state.selected?.manager;
                          obj.userSubmit = this.state.selected?.worker?.email;
                          obj.contractOrder = this.state.contractOrder;
                          bulkSubmit.push(obj);
                        }
              ...
              {!!this.state.batchCoupleDevicesError?.show && (
                <div
                  style={{
                    backgroundColor: '#fff',
                    position: 'fixed',
                ...
                >
                  <div style={{ fontWeight: 500, fontSize: '18px' }}>
                    {'Error on batch couple devices. Please check the list below.'}
                  </div>
                  <div style={{maxHeight: '350px', overflow: 'auto', height: '100%'}}>
                    <table className="table">
                      ...
                      <tbody>
                      {(this.state.batchCoupleDevicesError?.data || [])
                        .map((it, index) => {
                          return (
                            <tr
                              key={index}
                  ...
                  <button
                    ...
                    onClick={() => {
                      this.setState({ batchCoupleDevicesError: null });
                    }}
                  >
                    {'Close'}
                  </button>
...
class ItemCheck extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.props.data.tempDataChecks = this.props.data.tempDataChecks || {};
    this.state.setFormPosition = "none";
  }

  componentDidMount() {
    this.setState({ mapKey: Date.now() })
    this.setCheckCoupleTimeout();
  }
  ...
  setCheckCoupleTimeout() {
    ...
    if (!this.state.sn || !this.state.msn) {
      ...
      if (this.props.data && this.props.data?.tempDataChecks) {
        this.props.data.tempDataChecks.isCoupled = null;
        this.props.data.tempDataChecks.disableCoupleReason = null;
        this.props.data.tempDataChecks.disableDeCoupleReason = null;
      }
      return;
    }
    this.checkCoupleTimeout = setTimeout(async () => {
      if (!!this.state.sn && !!this.state.msn) {
        ...
        if (this.props.data?.tempDataChecks) {
          this.props.data.tempDataChecks.disableCoupleReason = null;
          this.props.data.tempDataChecks.disableDeCoupleReason = null;
        }
        if (!!device.id && this.state.msn === device.msn && this.state.sn === device.sn) {
          ...
          if (this.props.data && this.props.data?.tempDataChecks) {
            this.props.data.tempDataChecks.isCoupled = true;
            this.props.data.tempDataChecks.disableCoupleReason = null;
            this.props.data.tempDataChecks.disableDeCoupleReason = null;
          }
          ...
          if (this.props.data && this.props.data?.tempDataChecks) {
            this.props.data.tempDataChecks.isCoupled = false;
            if (!!device.msn) {
              this.props.data.tempDataChecks.disableDeCoupleReason = 'Device was coupled with other Meter SN! (' + device.msn + ')' ;
            } else {
              this.props.data.tempDataChecks.disableDeCoupleReason = 'Device has not been coupled.' ;
            }
          }
        } else {
          this.setState({isCoupled: null})
          if (this.props.data && this.props.data?.tempDataChecks) {
            this.props.data.tempDataChecks.isCoupled = null;
            if (!device.id || this.state.sn !== device.sn) {
              this.props.data.tempDataChecks.disableDeCoupleReason = 'Not exist MCU SN!';
            }
          }
        }
      }
  ...
  onboardingCheck = async () => {
    ...
    if (device.id) {
      ...
      if (this.state.action == 'onboardingCheck') {
        let log = (this.state.meterData || [])[0] || {};
        let isMatchMeterData = !!log.uid && !!log.msn;
        let errorMessage = null;
        if (!!log.msn && log.msn !== device.msn) {
          isMatchMeterData = false;
          errorMessage = 'Meter Data does not match Meter SN!'
        }
        if (device.status !== 'ONLINE' && !errorMessage) {
          errorMessage = 'Device is not online!';
        }
        await this.setState({check: (!!isMatchMeterData && device.status === 'ONLINE') ? 'pass' : 'fail', errorMessage})
      }
  ...
  coupleDevice = async () => {
    ...
    if (!!device.id && !device.msn) {
      ...
      if (!!device.id && !!device.msn && device.msn != this.state.msn) {
        Swal.fire(
          'Error!',
          'The MCU SN was coupled with other Meter SN (' + device.msn + ')',
          'error',
        );
      } else {
        Swal.fire(
  ...
  deCoupleDevice = async () => {
    ...
    if (!!device.id && device.msn === this.state.msn && device.sn === this.state.sn) {
      ...
      if (rp.success) {
        ...
        Swal.fire(
          'Error!',
          rp.message || rp.errorDescription || 'System error!',
          'error',
        );
      }
    } else {
  ...
  render() {
    ...
    console.info('item props', this.props)
    return (
      ...
      >
          ...
          <td>
            <input
              style={{
                ...
                width: '200px',
                paddingLeft: '10px',
                pointerEvents: ((this.props.currentLoggedIn !== this.props.currentWorker || !!this.props.job?.managerSubmit) ? 'none' : undefined),
              }}
              value={this.state.msn || ""}
              onChange={(event) => {

                this.props.data.msn = event.target.value;
                this.setState(
                  { msn: this.props.data.msn },
                  () => {
                    this.setCheckCoupleTimeout();
              ...
              onKeyPress={(event) => {
                if (event.type === "keypress" && event.key === "Enter") {
                  this.secondInput && this.secondInput.focus();
                }
              }}
            />
          </td>
          <td>
            <input
              ...
              value={this.state.sn || ""}
              onChange={(event) => {
                this.props.data.sn = event.target.value;
                this.setState(
                  { sn: this.props.data.sn },
                  () => {
                    this.setCheckCoupleTimeout();
              ...
              onKeyPress={(event) => {
                if (event.type === "keypress" && event.key === "Enter") {
                  this.props.onParentNextAction && this.props.onParentNextAction(event);
                }
              }}
            />
          </td>
          <td>
            <div
              className={`${(this.state.isCoupled === null || this.state.isCoupled === true) ? 'btn-text-disabled' : 'btn-text' } ml-10`}
              style={{pointerEvents: ((this.props.currentLoggedIn !== this.props.currentWorker || !!this.props.job?.managerSubmit) ? 'none' : undefined), }}
              onClick={async () => {
          ...
          <td>
            <div
              className={`${(this.state.isCoupled === null || this.state.isCoupled === false) ? 'btn-text-disabled': 'btn-text'} ml-10`}
              style={{display: 'flex', flexDirection: 'column', pointerEvents: ((this.props.currentLoggedIn !== this.props.currentWorker || !!this.props.job?.managerSubmit) ? 'none' : undefined), }}
              onClick={async () => {
            ...
            {(this.state.isCoupled === null || this.state.isCoupled === false) && (
              <div className='ml-10' style={{color: 'red'}}><span>{this.props.data.tempDataChecks?.disableDeCoupleReason}</span></div>
            )}
          </td>
          <td>
            <div
          ...
          <td>
            ...
            <div
              className="btn-text ml-10"
              onClick={async () => {
                /*if (!this.state.device?.uid || !!this.state.errorMessage) {
                  Swal.fire(
                    'Error!',
            ...
            >
              ...
              <i className="fa fa-remove"
                ...
                onClick={async () => {
                  ...
                  this.props.removeItem && this.props.removeItem();
                }}
              />
              )}
            </div>
