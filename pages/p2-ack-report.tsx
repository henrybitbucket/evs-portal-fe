...
class Homepage extends React.Component<IBasicPageProps, IDashboardPageState> {
  ...
  async componentDidMount() {
    ...
    const inSubGroups = await getMyGroupUsers({
      options: {
  ...
  render() {
    return (
      <TotPage title={'P2 Provisioning Job Sheet Report Ack'}>
            ...
            >
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{fontWeight: 400, fontSize: '18px', color: '#000000',}} className="site-title">{'P2 Provision'} <i className="fa fa-caret-right ml-5 mr-5"/> {'P2 Provisioning Job Sheet Report Ack'}</span>
                  <div style={{
                    display: 'flex',
                    flexDirection: 'row',
                    alignItems: 'center',
                ...
                <div className="table-wrap">
                  <table className="table"
                         style={{ marginTop: '15px', pointerEvents: !!this.state.openSubmitReport ? 'none' : null, }}>
                  ...
                  <div style={{
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        let res = await saveBulkReportAck(bulkSubmit)
                        if (res && res.success) {
                          Swal.fire(
                            'Information!',
  ...
  onboardingCheck = async () => {
    ...
    if (!!device.id && (!!this.state.msn && this.state.msn != device.msn)) {
      ...
      await this.setState({
        isLoading: false,
        check: null,
        errorMessage: 'the MCU and Meter ID searched are not coupled with each other, pls help to double check'
      });
  ...
  onStatusCheck = async () => {
    ...
    if (this.state.device?.id) {
      ...
      if (res && res.success) {
        ...
        await this.setState({
          showLastSubmit: { data: res.response },
          piFileUpLoads,
        ...
        await this.setState({
          showLastSubmit: { data: { ...this.state.device, timeSubmit: null, piFileUpLoads: [] } },
          show: true
        });
      }
          ...
          {!!this.state.runOnBoardingCheck && (
            <div
              ...
              onClick={async () => {
                ...
                this.setState({
                  action: 'onboardingCheck',
                  show: true,
                  showPhotoP1Test: true,
                  piFileUpLoadCurrentIndex: 0
                });
              }}
            >
